...
# Absolute imports (required now that we run from main.py)
...
def scrape_news(lang="en"):
    session = SessionLocal()
    try:
        # 1. History for Deduplication
        ...
        # 2. Determine Prompt based on Language
        tz = pytz.timezone('Asia/Kolkata')
        today_str = datetime.now(tz).strftime('%B %d, %Y')

        if lang == "hindi":
            search_prompt = (f"Search for today's date, {today_str}. Identify 5 weird, funny, or offbeat news stories "
                            f"worldwide. Focus on human absurdity, strange animal behavior, or bizarre inventions. "
                            f"Avoid serious politics or tragedy.")
        else:
            search_prompt = f"Find 5 unique, positive news stories from India today (achievements/tech/infra) for {today_str}."
        ...
        for item in news_items:
            ...
            if not is_dup:
                session.add(VideoJob(
                    ...
                    news_url=final_url, 
                    news_source=item.get('source', 'Web'),
                    status="pending", 
                    created_at=datetime.now()
                ))
                logger.info(f"ADDED ({lang}): {title}")
            else:
